    ...
    function getSessionToken() {
        ...
        $.ajax({
            ...
            if (response.response_code === 0) {
                ...
                alert("The pagee will be reloaded.");
                location.reload();
            }
        })
    }
...
　
        // display the muitiple choice answers
        for (var i = 0; i < gameObject.shuffledAnswerArray.length; i++) {
            var elementLabel = $("<label>");
            elementLabel.addClass("custom-control custom-radio");
            elementLabel.attr("Id", "custom-radio-" + (i + 1));

            var elementInput = '<input id="radio1" name="radio" type="radio" class="custom-control-input"';
...
　
            ...
            // Had to create the <input> element this way because a closing tag
            // </input> was also being generated which caused the text not to appear.
            // The <input> element does not have a closing tag </input>.
            ...
            */
            // set the "checked" property to true for the question
            if (i === 0) {
